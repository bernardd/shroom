defmodule ShroomWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: ShroomWeb.Gettext
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{"primary" => "btn-primary text-white", nil => "btn-primary btn-soft text-white"}
    ...
    assigns =
      ...
      |> assign(:custom_classes, assigns[:class])

    if rest[:href] || rest[:navigate] || rest[:patch] do
      ~H"""
      <.link class={[@base_classes, @custom_classes]} {@rest}>
        {render_slot(@inner_block)}
      </.link>
      """
    else
      ~H"""
      <button class={[@base_classes, @custom_classes]} {@rest}>
        {render_slot(@inner_block)}
      </button>
      """
    end
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div class="fieldset mb-2">
      <label>
        <span :if={@label} class="label mb-1 text-zinc-900">{@label}</span>
        <select
          id={@id}
          name={@name}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ShroomWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ShroomWeb.Gettext, "errors", msg, opts)
    end
  end
